...
describe('Item (e2e)', () => {
    ...
    const userRequest = (req: SupertestTest) =>
        req.set('Authorization', `Bearer ${userToken}`);
    const adminRequest = (req: SupertestTest) =>
    ...
    async function registerItemCategoryFontAdapter(
        ...
        const {
            body: { adapter: resAdapter },
        } = await adminRequest(
            request(app.getHttpServer()).post('/adapter'),
        ).send(adapter);

        // Faço outra requisição para criar uma fonte associada a esse adapter
        const {
            body: { font: resFont },
        } = await adminRequest(request(app.getHttpServer()).post('/font')).send(
            {
                ...font,
        ...
        const {
            body: { category: resCategory },
        } = await adminRequest(
            request(app.getHttpServer()).post('/category'),
        ).send(category);

        // Registrando o item
        const {
            body: { item: resItem },
        } = await adminRequest(request(app.getHttpServer()).post('/item')).send(
            {
                ...item,
                fontId: resFont.id,
    ...
    describe('(POST) /item', () => {
        it('Registra um item', async () => {
            ...
            const res = await adminRequest(
                request(app.getHttpServer()).post('/item'),
            )
                .send({
                    ...itemBody,
                    fontId: font.id,
        ...
        it('Tenta registrar um item com informações não permitidas', async () => {
            const missingArgumentsRes = await adminRequest(
                request(app.getHttpServer()).post('/item'),
            )
                .send({
            ...
            const extraArgumentsRes = await adminRequest(
                request(app.getHttpServer()).post('/item'),
            )
                .send({
                    ...itemBody,
    ...
    describe('(GET) /item/:id', () => {
        it('Pegar um item específico', async () => {
            const get_res = await userRequest(
                request(app.getHttpServer()).get(`/item/${item.id}`),
            ).expect(200);
        ...
        it('Tenta pegar item que não existe', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/item/200`),
            ).expect(404);
            validateError(res.body, 404);
        });
    });
    ...
    describe('(GET) /item', () => {
        it('Pegar vários itens (Com paginação)', async () => {
            ...
            const res = await userRequest(
                request(app.getHttpServer()).get(
                    `/item?page=${paginationInfo.page}&limit=${paginationInfo.limit}`,
                ),
            ).expect(200);
        ...
        it('Pegar vários itens (Com pesquisa)', async () => {
            // Checando a resposta do GET
            const res = await userRequest(
                request(app.getHttpServer()).get(`/item`),
            )
                .query({
                    name: 'Papel',
                })
    ...
    describe('(PATCH) /item', () => {
        it('Atualiza um item normalmente', async () => {
            // Atualiza a fonte
            const res = await adminRequest(
                request(app.getHttpServer()).patch(`/item/${item.id}`),
            )
                .send({
                    name: 'Mock Item',
                })
        ...
        it('Troca a fonte do item', async () => {
            ...
            // Validando se foi atualizado
            const getRes = await userRequest(
                request(app.getHttpServer()).get(`/item/${item.id}`),
            ).expect(200);
        ...
        it('Tenta acessar como usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).patch('/item/1'),
            ).expect(401);
        ...
        it('Tenta atualizar item que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch('/item/200'),
            ).expect(404);
        ...
        it('Passa informações que não existem', async () => {
            const unknownPropRes = await adminRequest(
                request(app.getHttpServer()).patch(`/item/${item.id}`),
            )
                .send({
            ...
            const notExistingCategoryRes = await adminRequest(
                request(app.getHttpServer()).patch(`/item/${item.id}`),
            )
                .send({
                    categoryId: 2000,
            ...
            const notExistingFontRes = await adminRequest(
                request(app.getHttpServer()).patch(`/item/${item.id}`),
            )
                .send({
                    fontId: 2000,
    ...
    describe('(DELETE) /item/:id', () => {
        it('Deleta um item', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete(`/item/${item.id}`),
            ).expect(200);
            ...
            const getItemRes = await adminRequest(
                request(app.getHttpServer()).get(`/item/${item.id}`),
            ).expect(404);
        ...
        it('Tenta acessar com token de usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).delete(`/item/1`),
            ).expect(401);
        ...
        it('Tenta deletar item que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete('/item/200'),
            ).expect(404);
