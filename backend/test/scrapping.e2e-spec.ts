...
describe('Authentication (e2e)', () => {
    ...
    async function registerFontAdapter(
        ...
        const res = await adminRequest(
            request(app.getHttpServer()).post('/font'),
        ).send({
            ...font,
    ...
    beforeEach(async () => {
        ...
        const resFontAdapterRegister = await registerFontAdapter(
            fontBody,
            adapterBody,
        );
        ...
        const {
            body: { category: category1Res },
        } = await adminRequest(
            request(app.getHttpServer()).post('/category'),
        ).send(category1Body);
        const {
            body: { category: category2Res },
        } = await adminRequest(
            request(app.getHttpServer()).post('/category'),
        ).send(category2Body);
        ...
        const {
            ...
            request(app.getHttpServer()).post('/scrapping'),
        ).send({
            fontId: font.id,
        });
    ...
    describe('(POST) /scrapping', () => {
        it('Deve mandar dois tipos de informações e fazer scrapping dos itens', async () => {
            ...
            const firstTypeRes = await adminRequest(
                request(app.getHttpServer()).post('/scrapping'),
            )
                .send({
                    fontId: font.id,
            ...
            const secondTypeRes = await adminRequest(
                request(app.getHttpServer()).post('/scrapping'),
            )
                .send({
                    font: font2Body,
            ...
            const font2 = secondTypeRes.body.jobs[0].font;
            const adapter2 = secondTypeRes.body.jobs[0].font.adapter;

            // Ele deve registrar a fonte e o adapter que foram enviados
            const getFontRes = await adminRequest(
                request(app.getHttpServer()).get(`/font/${font2.id}`),
            );
            ...
            const getAdapterRes = await adminRequest(
                request(app.getHttpServer()).get(`/adapter/${adapter2.id}`),
            );
        ...
        it('Manda informações que não são esperadas', async () => {
            const resWrongProps = await adminRequest(
                request(app.getHttpServer()).post('/scrapping'),
            )
                .send({
            ...
            const resMissingFields = await adminRequest(
                request(app.getHttpServer()).post('/scrapping'),
            )
                .send({
                    font: font,
    ...
    describe('(GET) /job/:id', () => {
        it('Deve pegar as informações de um job', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/job/${job1.id}`),
            );
        ...
        it('Tenta pegar um job que não existe', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get('/job/200'),
            ).expect(404);
        ...
        it('Pegar vários jobs (Com paginação)', async () => {
            ...
            const res = await userRequest(
                request(app.getHttpServer()).get(
                    `/job?page=${paginationInfo.page}&limit=${paginationInfo.limit}`,
                ),
            ).expect(200);
    ...
    describe('(DELETE) /job/:id', () => {
        it('Deleta um job', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete(`/job/${job1.id}`),
            ).expect(200);
            expect(res.body).toEqual(
                expect.objectContaining({
                    message: expect.any(String),
        ...
        it('Tenta deletar um job que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete('/job/200'),
            ).expect(404);
            validateError(res.body, 404);
        });
        ...
        it('Tenta acessar como usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).delete('/job/1'),
            ).expect(401);
            validateError(res.body, 401);
        });
