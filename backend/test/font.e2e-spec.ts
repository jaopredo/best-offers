...
describe('Font (e2e)', () => {
    ...
    describe('(POST) /font', () => {
        ...
        it('Passa informações que não existem', async () => {
            ...
            const notExistingAdapter = await adminRequest(
                request(app.getHttpServer()).post('/font'),
            )
                .send({
                    ...fontBody,
    ...
    describe('(GET) /font/:id', () => {
        it('Pegar uma fonte específica', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/font/${font1.id}`),
            ).expect(200);
            expect(res.body).toEqual(
                expect.objectContaining({
                    message: expect.any(String),
        ...
        it('Tenta pegar fonte que não existe', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/font/200`),
            ).expect(404);
    ...
    describe('(GET) /font', () => {
        it('Pegar várias fontes (Com paginação)', async () => {
            ...
            const res = await userRequest(
                request(app.getHttpServer()).get(
                    `/font?page=${paginationInfo.page}&limit=${paginationInfo.limit}`,
                ),
            ).expect(200);
        ...
        it('Pegar várias fontes (Com banco pesquisa)', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/font`),
            )
                .query({
                    name: 'Foo',
                })
    ...
    describe('(PATCH) /font/:id', () => {
        it('Atualiza uma fonte normalmente', async () => {
            // Atualiza a fonte
            const res = await adminRequest(
                request(app.getHttpServer()).patch(`/font/${font1.id}`),
            )
                .send({
                    name: 'Mock Test',
                })
        ...
        it('Troca o adapter da fonte', async () => {
            ...
            const {
                body: { adapter: substituteAdapter },
            } = await adminRequest(
                request(app.getHttpServer()).post('/adapter'),
            )
        ...
        it('Tenta acessar como usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).patch('/font/1'),
            ).expect(401);
        ...
        it('Tenta atualizar fonte que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch('/font/200'),
            ).expect(404);
        ...
        it('Passa informações que não existem', async () => {
            ...
            const notExistingAdapterRes = await adminRequest(
                request(app.getHttpServer()).patch(`/font/${font1.id}`),
            )
                .send({
                    adapterId: 2000,
    ...
    describe('(DELETE) /font/:id', () => {
        it('Deleta uma fonte', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete(`/font/${font1.id}`),
            ).expect(200);
        ...
        it('Tenta acessar com token de usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).delete(`/font/1`),
            ).expect(401);
        ...
        it('Tenta deletar fonte que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete('/font/200'),
            ).expect(404);
