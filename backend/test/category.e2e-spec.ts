...
describe('Category (e2e)', () => {
    ...
    describe('(POST) /category', () => {
        it('Deve registrar uma categoria', async () => {
            const categRes = await adminRequest(
    ...
    describe('(GET) /category/:id', () => {
        it('Pegar uma categoria específica', async () => {
            const getRes = await userRequest(
                request(app.getHttpServer()).get(`/category/${category1.id}`),
            ).expect(200);
        ...
        it('Tenta pegar categoria que não existe', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/category/200`),
            ).expect(404);
    ...
    describe('(GET) /category', () => {
        it('Pegar várias categorias (Com paginação)', async () => {
            ...
            const res = await userRequest(
                request(app.getHttpServer()).get(
                    `/category?page=${paginationInfo.page}&limit=${paginationInfo.limit}`,
                ),
            ).expect(200);
        ...
        it('Pegar várias categorias (Com informações de pesquisa)', async () => {
            // Checando a resposta do GET
            const res = await userRequest(
                request(app.getHttpServer()).get(`/category`),
            )
                .query({
                    name: '1',
                })
    ...
    describe('(PATCH) /category/:id', () => {
        it('Atualiza uma categoria', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch(`/category/${category1.id}`),
            )
                .send({
        ...
        it('Tenta acessar como usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).patch('/category/1'),
            ).expect(401);
        ...
        it('Tenta atualizar categoria que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch('/category/200'),
            )
                .send({
                    name: 'New Test LoL',
                })
    ...
    describe('(DELETE) /category/:id', () => {
        ...
        it('Tenta acessar com token de usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).delete(`/category/1`),
            ).expect(401);
        ...
        it('Tenta deletar categoria que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete('/category/200'),
            ).expect(404);
