...
describe('Adapter (e2e)', () => {
    ...
    describe('(POST) /adapter', () => {
        it('Registra vários tipos de adapter', async () => {
            const resFirstAdapter = await adminRequest(
            ...
            const resSecondAdapter = await adminRequest(
                request(app.getHttpServer()).post('/adapter'),
            )
                .send(secondTypeAdapter)
                .expect(201);
    ...
    describe('(GET) /adapter/:id', () => {
        it('Pegar um adapter específica', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/adapter/${adapter1.id}`),
            ).expect(200);
        ...
        it('Tenta pegar adapter que não existe', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/adapter/200`),
            ).expect(404);
    ...
    describe('(GET) /adapter', () => {
        it('Pegar vários adapters (Com paginação)', async () => {
            ...
            const res = await userRequest(
                request(app.getHttpServer()).get(
                    `/adapter?page=${paginationInfo.page}&limit=${paginationInfo.limit}`,
                ),
            ).expect(200);
        ...
        it('Pegar vários adapters (Com informações de pesquisa)', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).get(`/adapter`),
            )
                .query({
                    sep: '+',
                })
    ...
    describe('(PATCH) /adapter/:id', () => {
        it('Atualiza um adapter', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch(`/adapter/${adapter1.id}`),
            )
                .send({
                    sep: 'test-sep',
                })
        ...
        it('Tenta acessar como usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).patch('/adapter/1'),
            ).expect(401);
        ...
        it('Tenta atualizar adapter que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).patch('/adapter/200'),
            ).expect(404);
    ...
    describe('(DELETE) /adapter/:id', () => {
        it('Deleta um adapter', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete(`/adapter/${adapter1.id}`),
            ).expect(200);
        ...
        it('Tenta acessar com token de usuário', async () => {
            const res = await userRequest(
                request(app.getHttpServer()).delete(`/adapter/1`),
            ).expect(401);
        ...
        it('Tenta deletar adapter que não existe', async () => {
            const res = await adminRequest(
                request(app.getHttpServer()).delete('/adapter/200'),
            ).expect(404);
