...
/* DTO */
import {
    ItemPaginationQueryDto,
    ItemPostDto,
    ItemUpdateDto,
} from 'types/item/item.dto';
...
export class ItemController {
    ...
    async create(@Body() item: ItemPostDto) {
        ...
        if (!createdItem)
            throw new NotFoundException(
                'A fonte ou a categoria informados no corpo da requisição não foram encontrados',
            );
    ...
    async get(@Param('id', ParseIntPipe) id: number) {
        ...
        if (!foundItem)
            throw new NotFoundException('Item especificado não encontrado');
    ...
    async getAll(@Query() query: ItemPaginationQueryDto) {
        ...
        const { items, count } = await this.itemService.getAll(
            query.limit,
            query.page,
            where,
        );
    ...
    async patch(
        ...
        @Body() item: ItemUpdateDto,
    ) {
        const updatedItem = await this.itemService.update(id, item);

        if (!updatedItem)
            throw new NotFoundException(
                'Ou o item, ou a fonte, ou a categoria especificados não foram encontrados',
            );
