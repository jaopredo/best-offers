...
export class AuthController {
    ...
    async register(@Body() user: UserRegisterDto) {
        ...
        const registeredUser = await this.authService.registerUser(user, 'user')
    ...
    async registerAdmin(@Body() user: UserRegisterDto) {
        ...
        // Cria o usuário no banco
        const registeredUser = await this.authService.registerUser(user, 'admin')
    ...
    async login(@Body() payload: UserLoginDto) {
        ...
        if (!bcrypt.compareSync(payload.password, user.password)) {
            throw new UnauthorizedException('Senha incorreta, por favor, tente novamente')
        }
