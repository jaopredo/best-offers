...
export class JwtAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const request = context.switchToHttp().getRequest()  // Pego a request
        const token = this.extractTokenFromHeader(request)  // Extraio o token
        if (!token) {  // Se eu não possuir nenhum token
            throw new UnauthorizedException('Nenhum token fornecido, faça login antes de acessar esta rota');
        }
        try {
            ...
            throw new UnauthorizedException('Algo deu errado na autentificação, tente fazer login novamente')
        }
        return true;
    }
...
export class RolesGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!roles) {  // Se não tiver nenhuma, ele só passa automaticamente
            return true
        }
