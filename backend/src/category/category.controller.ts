...
export class CategoryController {
    ...
    async get(@Param('id', ParseIntPipe) id: number) {
        ...
        if (!foundCategory)
            throw new NotFoundException('Categoria solicitada não encontrada');
    ...
    async getAll(@Query() query: CategoryPaginationQueryDto) {
        const { categories, count } = await this.categoryService.getAll(
            query.limit,
            query.page,
            {
                name: query.name,
    ...
    async patch(
        ...
        if (!updatedCategory)
            throw new NotFoundException(
                'A categoria passada não foi encontrada',
            );
