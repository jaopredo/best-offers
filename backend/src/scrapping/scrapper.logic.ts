...
export class ScrapperLogic {
    ...
    async scrap(data: ScrapperProcessorData) {
        ...
        if (!url) {
            ...
            this.logger.error(
                `JOB ${id}.${font.id}.${category.id}: Erro na construção da URL`,
            );
            return;
        }
        ...
        $(`.${font.adapter.itemContainerClassName}`).each((_, element) => {
            const moneyRegex = /R\$\s*(\d{1,3}(\.\d{3})*|\d+),\d{2}/;

            // Pegando as informações de cada item
            const $element = $(element);
            const itemName = $element
                .find(`.${font.adapter.itemNameClassName}`)
                .text()
                .trim();
            const price = $element
                .find(`.${font.adapter.itemPriceClassName}`)
                .text()
                .trim()
                .match(moneyRegex);
            ...
            if (font.adapter.itemSellerClassName) {
                itemSeller =
                    $element
                        .find(`.${font.adapter.itemSellerClassName}`)
                        .text()
                        .trim() ?? null;
            }
            const itemURL = $element
                .find(`.${font.adapter.itemURLClassName}`)
                .attr('href');

            // Campos obrigatórios de ter
            if (Number.isNaN(itemPrice) || !itemURL || !itemName) {
                this.logger.error(
                    `JOB ${id}.${font.id}.${category.id}: Item rejeitado por não ter preço, url ou nome`,
                );
                return;
            }
            ...
            this.logger.log(
                `Job ${id}.${font.id}.${category.id}: Item adicionado`,
            );
        });
